...
abstract class DynamodbLogger(loggersConfig: LoggersConfig) extends Logger {
  // aws credentials
  implicit val region = Region(loggersConfig.dynamodbAwsRegion)
  val credentials = new Credentials(loggersConfig.dynamodbAwsAccessKeyId, loggersConfig.dynamodbAwsSecretAccessKey)
